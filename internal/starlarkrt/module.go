...
// Module provides the Starlark runtime as a member of the engine's "runtimes"
// group, keyed by its language so the engine can select it per function version.
var Module = fx.Module("starlarkrt",
	fx.Provide(
		fx.Annotate(
			provideRuntime,
...
func provideRuntime(p runtimeParams) engine.RuntimeEntry {
	return engine.RuntimeEntry{
		Language: engine.LanguageStarlark,
		Runtime: New(Config{
			Logger:       p.Logger,
			KV:           p.KV,
			Env:          p.Env,
